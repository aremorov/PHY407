...
for j in range(n):
    ...
    if monomer_array[new_monomer_pos[0], new_monomer_pos[1]] == 0:
        ...
        if distance_okay:
            ...
            if random() < np.exp(-(new_energy-energy)/T_array_2[j]):
                # make switch
                energy = new_energy
                monomer_coords = np.copy(new_monomer_coords)
                monomer_array = np.copy(new_monomer_array)

#to compute the energies and their standard deviation:
E_array_step_mean = []
energy_sd = []
step_width = 500000
for i in range(int(n/step_width)):
    E_array_step_mean.append(np.mean(energy_array[i*step_width:(i+1)*step_width]))
    energy_sd.append(np.std(energy_array[i*step_width:(i+1)*step_width]))
